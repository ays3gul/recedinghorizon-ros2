...
class ViewpointPlanning:
    ...
    def spawn_easy_occlusion(self):
        """
        Small box on the camera->ROI sight line, centred on the ROI front face.
        Hides ~1/4 of the ROI front face. Camera (Y~-0.05) looks toward the ROI
        (Y=-0.4); the box at Y=-0.25 sits between them, in front of the bunny
        (bunny Y>-0.475), so it blocks the view without intersecting the target.
        """
        self.sdf_spawner.spawn_sized_box(np.array([0.5, -0.25, 1.1]), 1, "small")

    def spawn_hard_occlusion(self):
        """
        Medium box, same sight line and centre as easy, larger size.
        Hides ~1/2 of the ROI front face. Difficulty increases via size only,
        keeping the comparison controlled (same position, monotonic occlusion).
        """
        self.sdf_spawner.spawn_sized_box(np.array([0.5, -0.25, 1.1]), 1, "medium")

    def spawn_extreme_occlusion(self):
        """
        Large box on the sight line plus a small box at a different depth.
        Hides ~3/4 of the ROI front face and adds a depth layer so the target
        stays partly occluded from several orbital angles. Still solvable: the
        camera can orbit to glimpse the ROI past the box edges, which is exactly
        what RH-NBV's occlusion handling is meant to exploit.
        """
        self.sdf_spawner.spawn_sized_box(np.array([0.5, -0.25, 1.1]), 1, "large")
        self.sdf_spawner.spawn_sized_box(np.array([0.5, -0.20, 1.1]), 2, "small")
